...
export default function LostAndFoundPage() {
  ...
  const handleSend = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSending(true);
    try {
      const response = await fetch('/api/lost-and-found/claims', {
        method: 'POST',
      ...
      if (!response.ok) throw new Error('Unable to send claim');
      setItems(current => current.map(item => item.id === selected.id ? { ...item, status: 'contacting', claimStatus: 'new' } : item));
      setSent(true);
    } catch {
      setContactError(copy.sendError);
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.list} aria-label={copy.eyebrow}>
        {items.filter(item => item.status !== 'returned').map((item, index) => {
          ...
          <section className={styles.contactSheet} role="dialog" aria-modal="true" aria-labelledby="lost-found-contact-title" onMouseDown={event => event.stopPropagation()}>
            {sent ? (
              ...
              <form onSubmit={handleSend}>
                ...
                <div className={styles.sheetActions}><button type="button" onClick={() => setSelected(null)}>{copy.close}</button><button type="submit" disabled={sending}>{copy.send} <ArrowUpRight size={16} /></button></div>
              </form>
            )}
          </section>
        </div>
