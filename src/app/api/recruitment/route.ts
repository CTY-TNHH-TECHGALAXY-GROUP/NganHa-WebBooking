...
// Hàm hỗ trợ loại bỏ dấu Tiếng Việt và các ký tự đặc biệt để tạo tên folder theo tên ứng viên
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Đặt tên folder theo tên ứng viên để Admin nhận biết trực quan trong Storage
    const rawFullName = formData.get('fullName')?.toString()?.trim() || 'ung-vien';
    const applicantSlug = removeVietnameseTones(rawFullName) || 'ung-vien';
    const folder = `${applicantSlug}-${Date.now()}`;
